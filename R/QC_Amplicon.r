    ...
    if (NCHR==1) {
	    ...
	    for (k in unique(Temp$Chr)) {
		    if (k==1) {
			    ...
			    lastbase=lastbase+tail(subset(Temp,Chr==lastchr)$BP, 1)
                lastchr=k
        ...
        for (k in unique(BinFrag$Chr)) {
            if (k==1) {
			    ...
			    lastbase=lastbase+tail(subset(BinFrag,Chr==lastchr)$BP, 1)
                lastchr=k
...
barplot(TOTAL_READS, main="Total Reads", cex.main=1.4, ylim=c(0,max(c(40000000, max(TOTAL_READS)+1000000))), col=samplecolours)
abline(20000000,0)
