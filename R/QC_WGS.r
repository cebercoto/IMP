...
barplot(TOTAL_READS, main="Total Reads", cex.main=1.4, ylim=c(0,max(c(50000000, max(TOTAL_READS)+1000000))), col=samplecolours)
abline(25000000,0)
barplot(MEAN_COVERAGE, main="Coverage", cex.main=1.4, ylim=c(0,max(c(80, max(MEAN_COVERAGE)))), col=samplecolours)
abline(40,0)
barplot(SD_COVERAGE, ylim=c(0,max(c(8, max(SD_COVERAGE)))), main="SD Coverage", cex.main=1.4, col=(samplecolours))
abline(3.5,0)
barplot(rbind(PCT_100X,PCT_10X,PCT_1X,STRAND_BALANCE), beside=T, ylim=c(0,1), main="Incl Percts", cex.main=1.4, col=brewer.pal(4, "Greys"))
abline(0.5,0)
abline(0.9,0)
legend("bottomright", c("% bases 100x", "% bases 10x", "% bases 1x", "Strand"), inset=.02, horiz=F, cex=1.4, fill=brewer.pal(4, "Greys"), bg="#ffffffb3")
barplot(rbind(PERCENT_DUPLICATION,PCT_EXC_OVERLAP,PCT_EXC_MAPQ,PCT_EXC_BASEQ), beside=T, ylim=c(0,1), main="Excl Percts", cex.main=1.4, col=brewer.pal(4, "Greys"))
abline(0.2,0)
abline(0.05,0)
legend("topright", c("% Dups", "% Overlap", "% LowMap", "% LowQ"), inset=.02, horiz=F, cex=1.4, fill=brewer.pal(4, "Greys"), bg="#ffffffb3")
